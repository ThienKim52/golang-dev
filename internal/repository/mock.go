...
func (m *MockLinkRepository) Save(ctx context.Context, code, url string, exp time.Duration) error {
	args := m.Called(ctx, code, url, exp)
	return args.Error(0)
}

func (m *MockLinkRepository) GetByCode(ctx context.Context, code string) (string, error) {
